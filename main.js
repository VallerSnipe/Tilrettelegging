// main.js (DEBUG-VERSJON MED DIALOGBOKSER)

const { app, BrowserWindow, ipcMain, net, shell, Notification, dialog } = require('electron');
const path = require('path');
let log; // Definer log variabel, men ikke konfigurer den ennå

try {
    dialog.showMessageBoxSync(null, { message: 'Debug: main.js starter. Importer er OK.' });
    ...
    function configureLogging() {
        dialog.showMessageBoxSync(null, { message: 'Debug: Før logging konfigureres.' });
        log = require('electron-log');
        log.transports.file.resolvePath = () => path.join(app.getPath('userData'), 'logs/main.log');
        dialog.showMessageBoxSync(null, { message: 'Debug: ETTER logging er konfigurert.' });
        log.info('--- Ny app-økt startet ---');
    }

    function checkForUpdates() {
        if (!log) return; // Ikke kjør hvis logging feilet
        dialog.showMessageBoxSync(null, { message: 'Debug: checkForUpdates() starter.' });
            ...
            response.on('end', () => {
                try {
                    ...
                    if (semver.gt(latestVersion, currentVersion)) {
                        ...
                        new Notification({ title: 'Ny versjon tilgjelig!', body: `Versjon ${latestVersion} er klar for nedlasting.`}).on('click', () => { shell.openExternal(release.html_url); }).show();
                    } else {
    ...
    function startServer() {
        serverApp.listen(PORT, () => {
            if (log) log.info(`Backend-server for utvikling startet på http://localhost:${PORT}`);
        });
    }
    
    function createWindow() {
        dialog.showMessageBoxSync(null, { message: 'Debug: createWindow() starter.' });
    ...
    app.whenReady().then(() => {
        dialog.showMessageBoxSync(null, { message: 'Debug: app.whenReady() er nådd.' });
        configureLogging();
        const isDev = !app.isPackaged;
    ...
    ipcMain.handle('api-request', async (event, { method = 'GET', endpoint, params, body }) => {
        // ... (IPC-koden er uendret)
        if (log) log.info(`IPC Mottatt: ${method} ${endpoint}`);
        try {
            ...
            if (log) log.error(`Feil i IPC-handler for ${endpoint}:`, error.message);
            return { error: error.message };
        }
    });

    app.on('window-all-closed', () => { if (process.platform !== 'darwin') app.quit(); });
    app.on('will-quit', () => { if (log) log.info('Appen avsluttes.'); });

} catch (err) {
    dialog.showErrorBox('KRITISK OPPSTARTSFEIL', `En alvorlig feil skjedde i main.js:\n\n${err.stack}`);
    app.quit();
}