// main.js (Endelig versjon med pålitelig dialogboks for oppdatering)
    ...
    request.on('response', (response) => {
        ...
        response.on('end', () => {
            try {
                ...
                if (!release.tag_name) {
                    log.warn('Svar fra GitHub manglet "tag_name".');
                    return;
                }
                ...
                if (semver.gt(latestVersion, currentVersion)) {
                    log.info('Ny versjon funnet! Viser dialogboks for oppdatering.');
                    
                    // ---- HER ER ENDRINGEN: Bytter fra Notification til dialog.showMessageBox ----
                    dialog.showMessageBox(mainWindow, {
                        type: 'info',
                        title: 'Oppdatering tilgjengelig',
                        message: `En ny versjon (${latestVersion}) er klar!`,
                        detail: `Du har versjon ${currentVersion}. Vil du gå til nedlastingssiden nå?`,
                        buttons: ['Ja, last ned nå', 'Senere'],
                        defaultId: 0, 
                        cancelId: 1   
                    }).then(result => {
                        if (result.response === 0) { // Brukeren klikket på den første knappen ("Ja")
