...
function getFaggrupper(sok = '') {
    try {
        ...
        console.error("Feil i getFaggrupper:", error);
        return { error: error.message };
    }
}

function getElevById(id) {
    try {
        const elev = db.prepare('SELECT * FROM elever WHERE elev_id = ?').get(id);
        if (!elev) {
            return { error: 'Elev ikke funnet.' };
        }
        ...
        console.error("Feil i getElevById:", error);
        return { error: error.message };
    }
}

function getFaggruppeDetails(faggruppeNavn) {
    try {
        return db.prepare('SELECT fagnavn, lærer FROM tilrettelegginger WHERE faggruppe_navn = ? LIMIT 1').get(faggruppeNavn);
    } catch (error) {
        console.error(`Feil i getFaggruppeDetails:`, error);
        return { error: error.message };
    }
}
...
function getEleverForFaggruppe(faggruppeNavn) {
    try {
        const query = `
            ...
            ORDER BY e.navn
        `;
        return db.prepare(query).all(faggruppeNavn);
    } catch (error) {
...
function addElev({ navn, klasse }) {
    try {
        const result = db.prepare('INSERT INTO elever (navn, klasse) VALUES (?, ?)')
                         .run(navn, klasse);
        return { id: result.lastInsertRowid, navn, klasse };
    } catch (error) {
        console.error(`Feil i addElev:`, error);
        return { error: error.message };
    }
}

function deleteElev(id) {
    try {
        const result = db.prepare('DELETE FROM elever WHERE elev_id = ?').run(id);
        return { changes: result.changes };
    } catch (error) {
        console.error(`Feil i deleteElev:`, error);
        return { error: error.message };
    }
}

function addTilrettelegging(data) {
    try {
        const { elev_id, faggruppe_navn, fagnavn, lærer } = data;
        const stmt = db.prepare(`
            INSERT INTO tilrettelegginger 
...
function deleteTilrettelegging(id) {
    try {
        const result = db.prepare('DELETE FROM tilrettelegginger WHERE tilrettelegging_id = ?').run(id);
        return { changes: result.changes };
    } catch (error) {
        console.error(`Feil i deleteTilrettelegging:`, error);
        return { error: error.message };
    }
}

function updateTilrettelegging(id, data) {
    try {
        ...
        console.error(`Feil i updateTilrettelegging:`, error);
        return { error: error.message };
    }
}

function bulkUpdateTilrettelegginger(elevId, { field, value }) {
    try {
        ...
        console.error(`Feil i bulkUpdateTilrettelegginger:`, error);
        return { error: error.message };
    }
}

function bulkUpdateKommentar(elevId, { kommentar }) {
    try {
        const query = `UPDATE tilrettelegginger SET kommentar = ? WHERE elev_id = ?`;
        const result = db.prepare(query).run(kommentar, elevId);
        return { changes: result.changes };
    } catch (error) {
        console.error(`Feil i bulkUpdateKommentar:`, error);
        return { error: error.message };
    }
}

function updateElev(id, { navn, klasse }) {
    try {
        ...
        console.error(`Feil i updateElev:`, error);
        return { error: error.message };
    }
}
