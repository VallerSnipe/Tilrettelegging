...
// Hele filen er nå en funksjon som mottar 'db'-objektet
// og returnerer et objekt med alle databasemetodene.
module.exports = function(db) {
    return {
        ...
        getEleverForFaggruppe(faggruppeNavn) {
            try { 
                const query = `SELECT e.elev_id, e.navn, e.klasse, t.faggruppe_navn, t.ekstra_tid, t.skjermet_plass, t.opplest_oppgave, t.kommentar FROM elever e JOIN tilrettelegginger t ON e.elev_id = t.elev_id WHERE t.faggruppe_navn = ? ORDER BY e.navn`; 
        ...
        addElev({ navn, klasse }) {
            try { 
                const result = db.prepare('INSERT INTO elever (navn, klasse) VALUES (?, ?)').run(navn, klasse); 
